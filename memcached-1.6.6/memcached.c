 ...
 */
//event_handler函数的核心函数，用于根据conn的类型来触发对应的请求，比如listen/parse_cmd等状态
...
//全局的事件处理函数，一旦某个FD有IO事件都会触发该函数的调用
 ...
 */
/*
初始化连接指针数组，数组大小是由settings.maxconns + headroom + next_fd;决定
*/
static void conn_init(void) {
...
//初始化一个连接，设置每个连接的的回调函数，并把对应的fd放到event_base中，在 event_handler 中设置对应IO事件的处理函数
/*
	1.把对应的sfd注册到对应的base中
	2.conns是全局变量，conns[sfd]的初始化、各种回调函数初始化
*/
 ...
 */
/* 1.tcp/udp 套接字初始化
   2.通过conn_new函数初始化listernfd的连接,设置listenfd的连接状态为conn_listening,并设置监听conn的各种回调函数
   3.初始化全局的listen_conn连接
   */
...
#else
	//main_base的初始化，该、main_base是监听来自客户端的请求
...
#else
	//核心函数，初始化memcached的worker线程,主线监听listenfd,工作线程监听notify_recieve_fd和客户端的fd
    //线程监听notify_recieve_fd，IO触发函数为thread_libevent_process
    //线程队列取出item初始化的conn中的fd,IO触发函数为event_handler
