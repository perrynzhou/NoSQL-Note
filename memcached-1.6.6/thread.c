 ...
 */
/*
setup_thread函数设置每个线程中的监听notify_receive_fd中的IO事件的event相关函数以及每个线程的连接的队列
*/
static void setup_thread(LIBEVENT_THREAD *me) {
    ...
    //当main线程有IO事件发生时候，会往notify_send_fd写标记数据，然后thread_libevent_process 函数通过标记数据来处理tcp连接
 ...
 */
//工作线程开始监听notify_receive_fd中的IO时间
 ...
 */
//主线程的监听套接字有IO事件时候，会往该线程的notify_send_fd中写标记数据，线程的event_base 监听notify_recieve_fd的IO事件，一旦有IO事件会触发thread_libevent_process执行
//notify_send_fd和notify_recieve_fd是一对pipe管道
static void thread_libevent_process(evutil_socket_t fd, short which, void *arg) {
    LIBEVENT_THREAD *me = arg;
    CQ_ITEM *item;
    char buf[1];
...
void dispatch_conn_new(int sfd, enum conn_states init_state, int event_flags,
	...
	//针对item进行对应的赋值，把fd和当前item的相关信息复制进去
    item->sfd = sfd;
...
//工作线程就是自己的队列取出item来查找conns的全局数组中的连接进行处理，conns[sfd]不为NULL，则sfd加入到线程中的event_base中的列表，同时设置event_handler的IO触发函数
